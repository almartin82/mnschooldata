# ==============================================================================
# LIVE Pipeline Tests for mnschooldata
# ==============================================================================
#
...
test_that("Minnesota DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Minnesota DOE URL and verify HTTP 200")
})
...
test_that("Can download Minnesota enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Minnesota enrollment file", {
  skip_if_offline()
...
test_that("mnschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- mnschooldata::get_available_years()
  ...
  tryCatch({
    raw <- mnschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- mnschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- mnschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- mnschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- mnschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- mnschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- mnschooldata:::get_cache_path(2024, "enrollment")
