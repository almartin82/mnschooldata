...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = mn.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pymnschooldata as mn
        assert hasattr(mn, 'fetch_enr')
        assert hasattr(mn, 'fetch_enr_multi')
        assert hasattr(mn, 'tidy_enr')
        assert hasattr(mn, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pymnschooldata as mn
        assert hasattr(mn, '__version__')
        assert isinstance(mn.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = mn.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = mn.fetch_enr(years['max_year'])
        # Total enrollment in Minnesota varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Minnesota should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = mn.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = mn.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pymnschooldata as mn
        years = get_test_years()
        df_single = mn.fetch_enr(years['max_year'])
        df_multi = mn.fetch_enr_multi([years['max_year']])
